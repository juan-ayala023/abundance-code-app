...
- **El pago no llegó al checkout en dos intentos.** ~~Pendiente de la landing.~~ **Resuelto por la landing el 14 de septiembre:** era CORS con `www.abundancecode.us`; corregido y desplegado en su backend. Ver [su respuesta](respuesta-landing-2026-09-14.md).
- **Compra real, gestión y cancelación con cuenta pagada.** **No se pueden probar:** Stripe está en modo real y no hay claves de test; una compra de prueba cobraría de verdad. Es una decisión del dueño (activar modo de pruebas en Stripe o asumir una compra real y reembolsarla). El contrato de canje y estado sí se verificó con filas creadas a mano, y hay un token de prueba para el flujo de `/activar` (en la respuesta, §2; **usadlo con una cuenta de Google que no sea la de cortesía**, porque el canje sustituye el acceso de esa cuenta).
- **Renovación: 49 $ de entrada con 30 días, después 14,99 $/mes.** No hay URL de continuación: el botón de la app sigue a `/pricing`, como pidió la landing. Quien canceló y vuelve paga 49 $ otra vez; es decisión de producto, no fallo.
- **A raíz de la respuesta se cambió en la app:** los enlaces legales apuntan a las páginas HTML de la landing (`/privacy`, `/terms`, `/refund`, `/disclaimer`) porque los PDF no eran los vigentes; «Gestionar mi suscripción» distingue «no hay suscripción de Stripe con tu correo» (404) de un fallo temporal; en `past_due` Mi Cuenta avisa de que la tarjeta se arregla desde el portal y no desde precios; y el plan `monthly` se enseña como «Suscripción mensual».
