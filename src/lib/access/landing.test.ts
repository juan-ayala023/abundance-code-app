...
describe("canjearToken", () => {
  it("devuelve el acceso y manda el secreto y el id de usuario", async () => {
    const fetchFalso = vi
      .fn()
      .mockResolvedValue(
        responder(200, { ...RESPUESTA, alreadyRedeemed: false }),
      );
    ...
    expect(r).toMatchObject({
      ok: true,
      acceso: { email: "maria@gmail.com", hasAccess: true },
    });
      ...
      vi
        ...
        .mockResolvedValue(
          responder(200, {
            ...
            redeemedAt: "2026-08-09T10:00:00Z",
          }),
        ),
    );
  ...
  it("distingue el token caducado y conserva el email para reenviarlo", async () => {
    ...
    expect(r).toEqual({
      ok: false,
      fallo: { motivo: "caducado", email: "maria@gmail.com" },
    });
  });
  ...
  it("distingue el token inexistente, que no tiene arreglo por reenvío", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(responder(404, { reason: "not_found" })),
    );
   ...
   */
  it.each([401, 503])(
    "trata el %i como falta de configuración",
    async (status) => {
      vi.stubGlobal(
        "fetch",
        vi.fn().mockResolvedValue(responder(status, { message: "no" })),
      );
  ...
  it("rechaza una respuesta que no cumple el contrato", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(responder(200, { email: "a@b.c" })),
    );
    vi.spyOn(console, "error").mockImplementation(() => {});

    const { canjearToken } = await importarModulo();
    expect(await canjearToken("tok", "user-1")).toEqual({
      ok: false,
      fallo: { motivo: "error" },
    });
  });
  ...
  it("entiende que no hay compra", async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          responder(200, { email: "x@y.z", hasAccess: false, status: "none" }),
        ),
    );
...
describe("urlDelPortalDeFacturacion", () => {
  it("devuelve la url de Stripe", async () => {
    const fetchFalso = vi
      .fn()
      .mockResolvedValue(
        responder(200, { url: "https://billing.stripe.com/x" }),
      );
    vi.stubGlobal("fetch", fetchFalso);

    const { urlDelPortalDeFacturacion } = await importarModulo();
    expect(await urlDelPortalDeFacturacion("maria@gmail.com")).toEqual({
      ok: true,
  ...
  it("distingue «no hay suscripción de Stripe» (404) de un fallo", async () => {
    // Un acceso creado a mano en su base no tiene `stripe_customer_id`: el
    // portal responde 404. No es temporal; la app no debe decir «reintenta».
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(responder(404, { message: "no" })),
    );
    const { urlDelPortalDeFacturacion } = await importarModulo();
    expect(await urlDelPortalDeFacturacion("nadie@x.com")).toEqual({
      ok: false,
      motivo: "no-encontrado",
    });
  });

  it("devuelve un fallo genérico si el backend no responde bien", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(responder(500, { message: "boom" })),
    );
    vi.spyOn(console, "error").mockImplementation(() => {});

    const { urlDelPortalDeFacturacion } = await importarModulo();
    expect(await urlDelPortalDeFacturacion("x@x.com")).toEqual({
      ok: false,
      motivo: "error",
    });
  });
